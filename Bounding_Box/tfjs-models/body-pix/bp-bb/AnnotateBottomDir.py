...
def bottomBoundingBox(data,h,w):
    #this function will get the highest and lowest leg y value 
    #and the left most and right most x value determined by either leg or foot
    #NOTE higher y values mean a lower postition
    legvals = [14,15,16,17,18,19,20,21]
    leftx=None
    rightx=None
    topy=None
    bottomy=None
...
def bounding_box_check(tl,br,w,h,picname):
    ...
    # next we test if the center of the box is within the center third of the 
    # image lastly for pants we make sure the bounding box is at least 10% of 
    ...
    if ((box_width - box_height)<epsilon and ((img_area*.2)<bb_area) and (box_center>center_third and box_center<(2*center_third))):
        return True
    else:
        return False
...
def main():
    ...
    for name in names:
        ...
        if(os.path.isfile(jsonname) and not(os.path.isfile(txtname))):
            ...
            top_left,bottom_right = bottomBoundingBox(d,h,w)

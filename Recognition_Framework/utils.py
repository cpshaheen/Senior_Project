...
def draw_bbox(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    ...
    for i in range(num_boxes[0]):
        ...
        if show_label:
            ...
            bb_items.append((bbox_mess,(c1,c2)))
...
def get_bbox(image, bboxes, attr_model, inv_mapping, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    num_classes = len(classes)
    image_h, image_w, _ = image.shape
    hsv_tuples = [(1.0 * x / num_classes, 1., 1.) for x in range(num_classes)]
    colors = list(map(lambda x: colorsys.hsv_to_rgb(*x), hsv_tuples))
    ...
    for i in range(num_boxes[0]):
        ...
        coor = out_boxes[0][i]
        # print(coor)
        coor[0] = int(coor[0] * image_h) # top left y
        coor[2] = int(coor[2] * image_h) # bottom right x
        coor[1] = int(coor[1] * image_w) # top left x
        coor[3] = int(coor[3] * image_w) # bottom right y
        # print(coor)
        crop_img = image.copy()
        crop_img = crop_img[int(coor[0]):int(coor[2]), int(coor[1]):int(coor[3])]
        crop_img = cv2.cvtColor(crop_img, cv2.COLOR_RGB2BGR)
        tags = attr_pred(crop_img,attr_model,inv_mapping)
        # cv2.imshow('crop',crop_img)
        tag_list.append(tags)
        bb_items.append(crop_img)
    
    return bb_items, tag_list
